    ...
    reader.onload = async (event) => {
      try {
        ...
        const formattedData = parsedData
          .filter((row) => row["Order ID"] !== "Platform unique order ID.")
          .map((row) => {
            let rawAmount = row["Order Amount"];
            let cleanAmount = Number(String(rawAmount || "0").replace(/[^0-9.-]+/g, ""));
            return {
              date: row["Order Create Time"] || row["Created Time"] || "-",
              orderId: String(row["Order ID"] || "-"),
              productName: String(row["Product Name"] || "Produk Tidak Diketahui"),
              // Mengambil Seller SKU, jika kosong ambil SKU ID (Platform SKU), jika kosong lagi beri "-"
              sku: String(row["Seller SKU"] || row["SKU ID"] || "-").trim(),
              quantity: Number(row["Quantity"] || 0),
              amount: isNaN(cleanAmount) ? 0 : cleanAmount,
              status: String(row["Order Status"] || "Unknown")
            };
          });
        const res = await fetch('/api/tiktok', {
          method: 'POST',
  ...
  const sortedOrders = useMemo(() => {
    ...
    if (sortConfig !== null) {
      sortableItems.sort((a, b) => {
        ...
        // Penanganan khusus untuk tanggal ("DD/MM/YYYY HH:MM:SS")
        if (sortConfig.key === 'date') {
          const parseDate = (dateStr: string) => {
            if (!dateStr || dateStr === "-") return new Date(0).getTime();
            const [datePart, timePart] = dateStr.split(" ");
            if (!datePart || !datePart.includes("/")) return new Date(0).getTime();
            const [day, month, year] = datePart.split("/");
                ...
                {currentOrders.length > 0 ? (
                  currentOrders.map((order, index) => (
                    <tr key={index} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-sm font-mono text-indigo-600 font-semibold">{order.sku}</td>
