...
export default function ShopeePage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      toast.error("Gagal mengambil data penjualan Shopee.");
    } finally {
      setIsFetching(false);
    }
  };
  ...
  const detectHeaderRow = (rows: unknown[][]) => {
    for (let i = 0; i < Math.min(rows.length, 40); i++) {
      ...
      if ((text.includes("nopesanan") || text.includes("orderid")) && (text.includes("namaproduk") || text.includes("produk") || text.includes("sku"))) return i;
    }
    return -1;
  };
  ...
  const normalizeRows = (parsedRows: Record<string, unknown>[]) => parsedRows
    ...
    .map((row): SalesRow | null => {
      const orderId = cleanOrderId(readCell(row, ["No. Pesanan", "No Pesanan", "Order ID", "ID Pesanan", "Nomor Pesanan"]));
      if (!orderId || orderId.toLowerCase().includes("platform")) return null;

      const marketplaceSku = String(readCell(row, ["Nomor Referensi SKU", "SKU Referensi", "Seller SKU", "SKU Penjual", "SKU", "SKU PRODUK"]) || "").trim();
      ...
      const quantity = Number(readCell(row, ["Jumlah", "Quantity", "Qty"]) || 1) || 1;
      const amount = parseCurrency(readCell(row, ["Total Pembayaran", "Total Harga", "Harga Setelah Diskon", "Subtotal", "Order Amount"]));
      const date = String(readCell(row, ["Waktu Pesanan Dibuat", "Tanggal Pesanan Dibuat", "Order Create Time", "Created Time", "Waktu Pemesanan"]) || "-").trim();
      const productName = String(readCell(row, ["Nama Produk", "Product Name", "Nama Barang", "Item Name"]) || "Produk Tidak Diketahui").trim();
      const variationName = String(readCell(row, ["Nama Variasi", "Variasi Produk", "Variation", "Variation Name"]) || "-").trim();
      const status = String(readCell(row, ["Status Pesanan", "Order Status", "Status"]) || "Unknown").trim();
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (rows.length === 0) throw new Error("Tidak ada data item Shopee yang valid.");
        ...
        toast.success("Data penjualan Shopee berhasil disimpan.");
        fetchOrders();
      } catch (error) {
        console.error(error);
        toast.error("Gagal membaca file. Upload file Data Pesanan / Order Detail Shopee dari Seller Center.");
      } finally {
  ...
  const exportData = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Shopee Sales Items");
    XLSX.writeFile(wb, "Shopee_Sales_Items.xlsx");
  };
  ...
  return (
    <main className="flex-1 bg-slate-50 min-h-screen text-slate-800 p-4 md:p-8 overflow-x-hidden">
      <header className="mb-6 flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-900 flex items-center gap-2"><Store size={24} /> Data Penjualan Shopee</h2>
          <p className="text-sm text-slate-500 mt-1">Upload file pesanan Shopee. Data disimpan sebagai item per produk agar multi-item order terbaca real.</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          {user?.role === "admin" && (
            <label className="bg-slate-900 text-white px-4 py-2.5 rounded-xl text-sm font-bold flex justify-center items-center gap-2 cursor-pointer hover:bg-slate-800">
              <Upload size={16} /> {isUploading ? "Memproses..." : "Upload Shopee Sales"}
          ...
          <table className="w-full min-w-[1180px] text-left">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {isFetching ? (
                ...
                <tr><td colSpan={8} className="px-4 py-12 text-center text-sm font-bold text-slate-400">Belum ada data penjualan Shopee.</td></tr>
              )}
            </tbody>
          </table>
        </div>
